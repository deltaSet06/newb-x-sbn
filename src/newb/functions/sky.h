...
vec3 renderEndSky(vec3 horizonCol, vec3 zenithCol, vec3 viewDir, float t) {
  vec3 sky = vec3(0.0, 0.0, 0.0);
    viewDir.y = smoothstep(-0.4, 1.6, abs(viewDir.y));
    viewDir.x += 0.0 * sin(80.0 * viewDir.y - t + viewDir.z);

    float a = atan2(viewDir.x, viewDir.z); // Calculates the spike angle

    float s = sin(a * 30.0 + t);
    s = s * s;
    s *= 1.7 + 0.7 * sin(a * 10.0 - 0.5 * t);
    float g = smoothstep(0.9 - s, -5.0, viewDir.y);

    // Color mixing based on angle and position
    float f = (2.0 * g + 1.0 * smoothstep(1.0, -0.1, viewDir.y));
    float h = (1.0 * g + 1.53 * smoothstep(1.2, -0.2, viewDir.y));
